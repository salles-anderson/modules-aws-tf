...
variable "rds_instances" {
  description = "Lista de identificadores de instâncias RDS para gerenciar. Se vazio, usa tags para descoberta."
  type        = list(string)
  default     = []
}

variable "resource_tag_key" {
  description = "Chave da tag para descoberta automática de instâncias RDS."
  type        = string
  default     = null
}

variable "resource_tag_value" {
  description = "Valor da tag para descoberta automática de instâncias RDS."
  type        = string
  default     = null
}
...
variable "stop_schedule" {
  description = "Expressão cron para parar as instâncias (ex: cron(0 18 ? * MON-FRI *) = 18h seg-sex)."
  type        = string
...
variable "start_schedule" {
  description = "Expressão cron para iniciar as instâncias (ex: cron(0 8 ? * MON-FRI *) = 8h seg-sex)."
  type        = string
...
variable "enable_restop_schedule" {
  description = "Habilita schedule adicional para re-parar instâncias que foram auto-iniciadas após 7 dias. Recomendado para ambientes que ficam parados por longos períodos."
  type        = bool
  default     = false
}

variable "restop_schedule" {
  description = "Expressão cron para verificar e re-parar instâncias (ex: cron(0 19 ? * MON-FRI *) = 19h seg-sex)."
  type        = string
