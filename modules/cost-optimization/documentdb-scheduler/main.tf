locals {
  ...
  lambda_function_name = coalesce(var.lambda_function_name, "${var.project_name}-docdb-scheduler")

  stop_input = jsonencode({
    action    = "stop"
    clusters  = var.documentdb_clusters
...
data "aws_iam_policy_document" "lambda_docdb" {
  statement {
    sid = "DocumentDBOperations"
    actions = [
...
resource "aws_iam_role_policy" "lambda_docdb" {
  name   = "documentdb-scheduler-policy"
  role   = aws_iam_role.lambda.id
  policy = data.aws_iam_policy_document.lambda_docdb.json
}
...
resource "aws_lambda_function" "this" {
  ...
  description      = "Lambda para stop/start agendado de clusters DocumentDB"
...
resource "aws_scheduler_schedule" "stop" {
  name        = "${var.project_name}-docdb-stop"
  description = "Para clusters DocumentDB fora do horário comercial"
...
resource "aws_scheduler_schedule" "start" {
  name        = "${var.project_name}-docdb-start"
  description = "Inicia clusters DocumentDB no início do horário comercial"
  state       = var.enable_schedule ? "ENABLED" : "DISABLED"
...
resource "aws_scheduler_schedule" "restop" {
  ...
  name        = "${var.project_name}-docdb-restop"
  description = "Re-para clusters DocumentDB que foram auto-iniciados após 7 dias"
  state       = var.enable_schedule ? "ENABLED" : "DISABLED"
